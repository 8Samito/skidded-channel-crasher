...
async function raid(token, id, delay) {
    ...
    await axios.post(`https://discord.com/api/v8/channels/${id}/messages`, { "content": `${message}` }, {
        ...
        if (response.data.id) {
            mensagens++
            console.log(colors.green(`[+] `) + `${mensagens} message sent!` + colors.yellow(` [Awaiting ${delay}ms]`))
            await sleep(delay)
            raid(token, id, delay)
        } else {
            console.log(colors.red(`[-] `) + `Error sending message.`)
        }
    }).catch(err => {
        if (err.response.status === 429) {
            ...
            console.log(colors.red(`[SPAMMER] Error sending message. ${err}`))
        }
    })
